...
where
    ...
    F: Fn(X) -> Result<X, ComputableError> + Send + Sync + 'static,
{
    pub fn new(state: X, compute_prefix: B, refine: F) -> Self {
        Self {
    ...
    fn snapshot_prefix(&self, snapshot: &mut BaseSnapshot<X>) -> Result<Prefix, ComputableError> {
        ...
        let prefix = (self.compute_prefix)(&snapshot.state)?;
    ...
    F: Fn(X) -> Result<X, ComputableError> + Send + Sync + 'static,
{
    ...
    fn get_prefix(&self) -> Result<Prefix, ComputableError> {
        ...
        let prefix = self.snapshot_prefix(&mut snapshot)?;
        Ok(prefix)
    }
    ...
    fn refine(&self) -> Result<(), ComputableError> {
        ...
        if next_state == previous_state {
            if previous_prefix.width_exponent() == XI::NegInf {
                return Ok(());
            }
            return Err(ComputableError::StateUnchanged);
        }

        let next_prefix = (self.compute_prefix)(&next_state)?;
...
impl Node {
    ...
    /// are only needed when prefix *changes* during refinement (via
    /// `set_prefix`).
    pub fn get_prefix(&self) -> Result<Prefix, ComputableError> {
        if let Some(prefix) = self.cached_prefix() {
            return Ok(prefix);
        }
        let prefix = self.op.compute_prefix()?;
        {
