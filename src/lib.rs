...
pub use binary::{Binary, BinaryError, Exponent, ExtendedBinary, ExtendedBinaryError};
    ...
    F: Fn(X) -> X,
{
    ...
    pub fn neg(
        self,
    ) -> Computable<X, impl Fn(&X) -> Result<Bounds, ComputableError>, impl Fn(X) -> X> {
        let Computable {
            state,
            bounds: base_bounds,
            refine,
